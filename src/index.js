...
class App extends Component {
  ...
  plusThree = () => {
    let action = { type: 'plusThree' };
    store.dispatch(action);
  };

  minusFive = () => {
    let action = { type: 'minusFive' };
    store.dispatch(action);
  };
  ...
  render() {
    return (
      <div className="Counter">
        ...
        <button onClick={this.props.plusThree}>+3</button>
        <button onClick={this.props.minusFive}>-5</button>
      </div>
    );
  }
}
